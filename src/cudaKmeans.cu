...
"First (single): dataset, an [mxn] matrix. m must be a multiplier of 128 and n equal or smaller of 1024\n"
...
void errorMessage2(int Objects, int attributes, int numCluster){
  ...
  if((Objects % NUMBER_OF_SUBMATRICES) != 0 || attributes > BlockSize || numCluster >= Objects){
    mexErrMsgTxt(help);
  }
}
